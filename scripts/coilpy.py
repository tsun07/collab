...
def plot_vmec_surface(wout, ns=-1, plottype='cross-section',zeta=0.0, zeta1=2*np.pi,
                         c=(1,0,0),style='-', marker='.', width=2.0,lbl='VMEC_surface',npol=128,ntor=128, prange='full'):
    '''
    ...
    if plottype=='cross-section' : #plot cross-section
        ...
        if plt.get_fignums():
            ...
            fig, ax = plt.subplots()
        if prange == 'full':
            ax.plot(r, z, color=c,linewidth=width,linestyle=style,label=lbl, marker=marker)
            return r, z
        elif prange == 'upper':
